...
class DiceRoller():
    # eval() or "".join() seem to have an issue beyond this point
    __DICE_MAX = 2900
    ...
    def __construct_output(self, roll_input):
        """construct roll output"""
        # counter to prevent rediculouse amounts of random numbers being generated
        total_dice = 0
        ...
        for indx, item in enumerate(roll_input):
            ...
            if re.match(check_roll, item) is not None:
                ...
                # prevent rolling of too many dice
                if total_dice > self.__DICE_MAX:
    ...
    def handle_rolls(self, user_in: str, r_type: str = 's'):
        ...
        try:
            ...
            return 'The amount of dice being rolled exceeded the maximum allowed. Please try again with less dice.'
        except Exception as e:
